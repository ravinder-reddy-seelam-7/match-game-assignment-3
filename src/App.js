...
class App extends Component {
  ...
  getImage = () => {
    const imageIndex = Math.ceil(Math.random() * 30) - 1
    this.setState({item: imagesList[imageIndex]})
  }
      ...
      <div className="game-over-container">
        <div className="game-over-card">
          <img
            ...
            className="trophy"
          />
          <p className="game-over-card-text">YOUR SCORE</p>
          <h1 className="game-over-card-text">{score}</h1>
          <button type="button" className="play-again" onClick={this.playAgain}>
            <img
  ...
  showPlayView = () => {
    const {item, activeTabId} = this.state
    return (
      <div className="sec-container">
        <img src={item.imageUrl} alt="match" className="big-image" />
        <ul className="tabs-container">
          {tabsList.map(eachItem => (
            <li className="display-text" key={eachItem.tabId}>
              {eachItem.tabId === activeTabId ? (
                <button
                  type="button"
                  className="active-tab-button"
                  onClick={this.clickingTab}
                  id={eachItem.tabId}
                >
      ...
      <div className="bg-container">
        <ul className="nav-container">
          ...
          <li className="score-timer-container">
            ...
            />
            <p className="span-el">{time} sec</p>
          </li>
        </ul>
        {gameOver ? this.showGameOverView() : this.showPlayView()}
      </div>
    )
  }
}
